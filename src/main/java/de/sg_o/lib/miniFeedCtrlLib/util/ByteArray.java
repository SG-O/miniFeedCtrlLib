...
public class ByteArray {
    ...
    /**
     * Convert a int array to a hexadecimal string.
     * @param ints The int array to convert.
     * @return The hexadecimal representation of that array.
     */
    public static String intsToHex(int[] ints) {
        if (ints == null) return "";
        char[] hexChars = new char[ints.length * 8];
        for (int i = 0; i < ints.length; i++) {
            int v = ints[i];
            hexChars[i * 8] = HEX_ARRAY[(v >>> 28) & 0x0F];
            hexChars[i * 8 + 1] = HEX_ARRAY[(v >>> 24) & 0x0F];
            hexChars[i * 8 + 2] = HEX_ARRAY[(v >>> 20) & 0x0F];
            hexChars[i * 8 + 3] = HEX_ARRAY[(v >>> 16) & 0x0F];
            hexChars[i * 8 + 4] = HEX_ARRAY[(v >>> 12) & 0x0F];
            hexChars[i * 8 + 5] = HEX_ARRAY[(v >>> 8) & 0x0F];
            hexChars[i * 8 + 6] = HEX_ARRAY[(v >>> 4) & 0x0F];
            hexChars[i * 8 + 7] = HEX_ARRAY[v & 0x0F];
        }
        return new String(hexChars);
    }
